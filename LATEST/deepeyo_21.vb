...
Public Class Form1
    ...
    Public square_brush As New SolidBrush(Color.FromArgb(255, 235, 105))
    Public square_pen As New Pen(Color.FromArgb(190, 175, 80), 3) With {.LineJoin = Drawing2D.LineJoin.Round}
    Public triangle_brush As New SolidBrush(Color.FromArgb(250, 120, 120))
    Public triangle_pen As New Pen(Color.FromArgb(190, 90, 90), 3) With {.LineJoin = Drawing2D.LineJoin.Round}
    Public pentagon_brush As New SolidBrush(Color.FromArgb(120, 140, 250))
    Public pentagon_pen As New Pen(Color.FromArgb(90, 110, 190), 3) With {.LineJoin = Drawing2D.LineJoin.Round}
    Public minimap As Rectangle
    Private respawn_box As Boolean = False
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Catch ex As Exception
            Close()
        End Try
        shapes.Add(New Shape(0, 0, 0, 25, square_brush, square_pen, 4))
        shapes.Add(New Shape(100, 0, 0, 25, triangle_brush, triangle_pen, 3))
        shapes.Add(New Shape(-100, 0, 0, 35, pentagon_brush, pentagon_pen, 5))
        shapes.Add(New Shape(-300, 300, 0, 55, pentagon_brush, pentagon_pen, 5))
        width_ = canvas.Width
    ...
    Private Sub game_Tick(sender As Object, e As EventArgs) Handles game.Tick
        Try
            ...
            Dim ls As Integer = 0, ps As Integer = 0
            players.Clear()
            dead_players.Clear()
            bullets.Clear()
            dead_bullets.Clear()
            ...
            For Each o As String In objects
                ...
                If o.StartsWith("pb") Then
                    ...
                    If pp("a") = "0" Then
                        If respawn_box = False Then
                            respawn_box = True
                            If MessageBox.Show("Do you want to respawn?", "Respawn", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                                connection.Close()
                                connection = New Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp)
                                connection.Connect(New IPEndPoint(IPAddress.Parse(server), 35656))
                                stream = New NetworkStream(connection)
                ...
                ElseIf o.StartsWith("tr") Then
                    shapes.Add(New Shape(pp("x"), pp("y"), pp("d"), 15, triangle_brush, triangle_pen, 3))
                ElseIf o.StartsWith("sq") Then
                    shapes.Add(New Shape(pp("x"), pp("y"), pp("d"), 15, square_brush, square_pen, 4))
                ElseIf o.StartsWith("pn") Then
                    shapes.Add(New Shape(pp("x"), pp("y"), pp("d"), 23, pentagon_brush, pentagon_pen, 5))
                End If
            Next
            ...
            g.SmoothingMode = Drawing2D.SmoothingMode.HighSpeed
            g.FillRectangle(gui_brush, 0, 0, CInt((ps / ls) * width_), 15)
            If respawn_box = True Then g.FillRectangle(gui_brush, 0, 0, width_, height_)
            ...
            s.Stop()
            Me.Text = "deepeyo | " & CInt(1000 / s.ElapsedMilliseconds).ToString & " fps"
        Catch ex As Exception
        End Try
    End Sub
    ...
    Public Sub RenderShape(n As Shape, g As Graphics, offset As Point)
        Dim center As New Point(n.X, n.Y)
        Dim ps As New List(Of Point)
        For i = n.Direction To 360 + n.Direction Step 360 / n.Vertices
            ps.Add(RotatePoint(New Point(n.X + n.Size, n.Y), center, i) - offset)
        Next
        g.FillPolygon(n.Brush, ps.ToArray)
        g.DrawPolygon(n.Pen, ps.ToArray)
    End Sub
    ...
    Public Class Shape
        ...
        Public Sub New(x As Single, y As Single, dir As Single, size As Single, brush As SolidBrush, pen As Pen, verts As Integer)
            Me.X = x
            Me.Y = y
            Me.Size = size
            Direction = dir
